...
class Budged_request_model extends CI_Model
{
    ...
    // datatables
    function json() {
        $this->datatables->select('budged_request.id_req, budged_request.date_req, 
        ref_person.realname, ref_objective.objective, budged_request.title, 
        FORMAT(budged_request.budged_req, 0, "de_DE") AS budged_req, budged_request.comments, budged_request.id_country, 
        budged_request.id_person, budged_request.id_objective, budged_request.flag');
        ...
        if ($lvl == 7){
            ...
            $this->datatables->add_column('action', anchor(site_url('budged_request/read/$1'),'<i class="fa fa-th-list" aria-hidden="true"></i>', array('class' => 'btn btn-info btn-sm')) ."
            ...
            $this->datatables->add_column('action', anchor(site_url('budged_request/read/$1'),'<i class="fa fa-th-list" aria-hidden="true"></i>', array('class' => 'btn btn-info btn-sm')) ."
                ...
                ".anchor(site_url('budged_request/delete/$1'),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Confirm deleting sample : $1 ?\')"'), 'id_req');
        }
      ...
      if ($lvl == 7){
          $this->datatables->add_column('action', '', 'id_reqdetail');
            ...
            $this->datatables->add_column('action', '<button type="button" class="btn_edit_det btn btn-info btn-sm" aria-hidden="true"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>'." 
                ".anchor(site_url('Budged_request/delete/$1'),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Confirm deleting sample : $1 ?\')"'), 'id_reqdetail');
        }
      return $this->datatables->generate();
  }
