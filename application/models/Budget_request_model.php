...
class budget_request_model extends CI_Model
{
    ...
    function json() {
        ...
        if ($lvl == 7){
            $this->datatables->add_column('action', anchor(site_url('budget_request/read/$1'),'<i class="fa fa-th-list" aria-hidden="true"></i>', array('class' => 'btn btn-info btn-sm')), 'id_req');
        }
        else if (($lvl == 2) | ($lvl == 3)){
